...
public enum Command {
    ...
    SYSTEM_PREFERENCES("ab.systemprefs",
            ...
            input -> {
                ...
                Object sender = input.getSender();
                MessageManager.getLayout(mi.getMessages(), "AdvancedBan.SystemPrefs",
                        "TIME", calendar.get(Calendar.HOUR_OF_DAY) + ":" + calendar.get(Calendar.MINUTE),
                        "UUID", String.valueOf(mi.getInternUUID(sender)))
                        .forEach(line -> mi.sendMessage(sender, line));
                if (input.hasNext()) {
                    String target = input.getPrimaryData();
                    MessageManager.getLayout(mi.getMessages(), "AdvancedBan.SystemPrefsTarget",
                            "TARGET", target,
                            "INTERN", String.valueOf(mi.getInternUUID(target)),
                            "FETCHED", String.valueOf(UUIDManager.get().getUUID(target)))
                            .forEach(line -> mi.sendMessage(sender, line));
                }
            },
            null,
                ...
                if (input.hasNext()) {
                    if (input.getPrimaryData().equals("reload")) {
                        ...
                        if (Universal.get().hasPerms(sender, "ab.help")) {
                            MessageManager.getLayout(mi.getMessages(), "AdvancedBan.Help")
                                    .forEach(line -> mi.sendMessage(sender, line));
                        } else {
                            MessageManager.sendMessage(sender, "General.NoPerms", true);
                        }
